...
public class UserDAOTest
{
    ...
    {
        List<User> users = dao.findByLastname("Simpson");
        User homer = new User(1, "homer", "simpson", "homer", "**********");
        User marge = new User(2, "marge","simpson", "marge", "**********");
        
        Assert.assertTrue(users.contains(homer));
        Assert.assertTrue(users.contains(marge));
    }
    
    @Test
    public void testFindByUsername()
    {
    	User marge = dao.findByUsername("marge");
    	
    	Assert.assertEquals("marge", marge.getUsername());
    }
